...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        for (Long item: items) {
            ...
            double sumOfSquareOfItemVector = 0.0;
            for(Map.Entry<String, Double> itemVector : iv.entrySet()){
                sumOfSquareOfItemVector += Math.pow(itemVector.getValue(), 2);
            }
            double sqrtOfSumOfSquareOfItemVector = Math.sqrt(sumOfSquareOfItemVector);

            double sumOfSquareOfUserProfile = 0.0;
            for(Map.Entry<String, Double> userProfile : userVector.entrySet()){
                sumOfSquareOfUserProfile += Math.pow(userProfile.getValue(),2);
            }
            double sqrtOfSumOfSquareOfUserProfile = Math.sqrt(sumOfSquareOfUserProfile);

            double dotProduct = 0.0;
            for(Map.Entry<String, Double> userProfileVector : userVector.entrySet()){
                if(iv.containsKey(userProfileVector.getKey())){
                    dotProduct += (iv.get(userProfileVector.getKey()) * userProfileVector.getValue());
                }
            }
            double prodOfBothSqrt = sqrtOfSumOfSquareOfItemVector * sqrtOfSumOfSquareOfUserProfile;

            double score = 0.0;
            if(prodOfBothSqrt != 0){
                score = dotProduct / prodOfBothSqrt;
            }
