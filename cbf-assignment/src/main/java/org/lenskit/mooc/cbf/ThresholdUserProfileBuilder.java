    ...
    public Map<String, Double> makeUserProfile(@Nonnull List<Rating> ratings) {
        ...
        for (Rating r: ratings) {
            if (r.getValue() >= RATING_THRESHOLD) {
                ...
                Map<String, Double> tagVector = model.getItemVector(itemId);
                for(Map.Entry<String, Double> tagv: tagVector.entrySet()){
                    if(!profile.containsKey(tagv.getKey())){
                        profile.put(tagv.getKey(), tagv.getValue());
                    }else{
                        profile.put(tagv.getKey(), tagv.getValue()+profile.get(tagv.getKey()));
                    }
