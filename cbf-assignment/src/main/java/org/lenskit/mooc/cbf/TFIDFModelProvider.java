...
public class TFIDFModelProvider implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            //work = term frequency = how many times 1 tag appears
            Map<String, Double> work = new HashMap<>();
            HashSet<String> uniqueTagsPerItem = new HashSet<>();
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                if(!uniqueTagsPerItem.contains(tag)){
                    uniqueTagsPerItem.add(tag);
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            // TODO Convert this vector to a TF-IDF vector
                for(Map.Entry<String, Double> tagFreq : tv.entrySet() ){
                    double tfIdf = docFreq.get(tagFreq.getKey())*tagFreq.getValue();
                    tagFreq.setValue(tfIdf);
            ...
            double sqrtOfSumOfSquares = Math.sqrt(sumOfSquares);
            for(Map.Entry<String, Double> tagFreq : tv.entrySet()){
                double normalizedTfIdf = tagFreq.getValue() / sqrtOfSumOfSquares;
                tagFreq.setValue(normalizedTfIdf);
            }
